...
function Record({ title = "", btns = [], setUserData, args = [] }) {
  const setButtonData = (id) => {
    setUserData(id);
  };
  return (
      ...
      <div className="ml-auto flex flex-row p gap-10">
        {btns.map((item, index) => {
          return (
            <div
              onClick={() => {
                setButtonData(args[index]);
              }}
              key={index}
              className="font-sans rounded-lg px-3 bg-[#fff] text-black cursor-pointer py-1 duration-200  "
            >
              {item.text}
            </div>
          );
